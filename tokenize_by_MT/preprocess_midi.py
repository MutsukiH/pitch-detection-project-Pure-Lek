...
def prep_custom_midi(custom_midi_root, output_dir, valid_p = 0.1, test_p = 0.2):
    ...
    # test_dir = os.path.join(output_dir, "test")
    # os.makedirs(test_dir, exist_ok=True)
    output_csv = './' + "tokenize" + custom_midi_root.split("/")[-1] + ".csv"
    ...
    with open(output_csv, mode='w', newline='') as file:
        ...
        writer.writerow(['File Name wav','label'])
        for piece in os.listdir(custom_midi_root):
            ...
            # print(mid)
            f_name      = 'wav_' + piece.split(".")[0].split('_')[-1] + ".wav"
            ...
            # save to csv
            writer.writerow([f_name, prepped])
...
def parse_args():
    ...
    parser.add_argument("-output_dir", type=str, default="./model_ref_MT/data_tokenize", help="Output folder to put the preprocessed midi into.")
