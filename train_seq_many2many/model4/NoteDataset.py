...
class NoteDataset(Dataset):
    ...
    def split_melspectrogram(self, signal):
        ...
        total_size = signal.size(1) # เอาความยาวเวลาทั้งหมดของไฟล์นั้น ๆ มา อยู่ตน. 1 เพราะรูปร่างคือ (1, เวลา, n_mel)
        ...
        split_tensors = torch.split(signal, split_size, dim=1) # แบ่ง mel-spectrogram ออกตามเวลาที่กำหนด
        ...
        if remainder > 0: # เช็คว่าเหลือเศษไหม
            ...
            padding_size = split_size - last_tensor.size(1) # คำนวนว่าต้อง padding อีกเท่าไหร่
            # กำหนดว่าจำทำ padding ที่มิติไหนบ้าง โดยไม่ทำกับมิติที่ 0 - 2 แต่ทำกับมิติที่ 3 ตามจำนวน padding size ที่คำนวนไว้
...
def create_data_loader(data, batch_size):
    ...
    for batch in range(1, num_batch+1): # วนตามจำนวน batch ที่คำนวนได้
        ...
        labeltensor_batch = pad_sequence(labeltensor_list[start:batch_size*batch], padding_value=-1,batch_first=True)
