...
class DecoderRNN(nn.Module):
    ...
    def forward(self, encoder_outputs, encoder_hidden, target_tensor=None):
        ...
        for i in range(1, MAX_LENGTH+1):
            ...
            _, topi = decoder_output.topk(1) # เอาตำแหน่งของค่าที่มากที่สุดออกมา
            if topi == n_note - 1: # ถ้าผลเป็น eos 
                break # ให้ออกจากลูป
            else: # ถ้าไม่เป็น eos 
                decoder_input[0][0][topi] = 1 # ให้เปลี่ยนตำแหน่งนั้นใน [0, 0, ..., 0] เป็น 1 เพื่อเป็น input ในรอบถัดไป
